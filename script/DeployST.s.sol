...
import "../src/storage/TokenStorage.sol";

/**
 * @title DeploySTScript
 * @dev Security Token deployment script with meta-transaction capability
 * Based on DeployAllFixed.s.sol but includes EIP-2612 permit functionality
 */
contract DeploySTScript is Script {
    ...
    function _phaseOne_DeployBaseInfrastructure(address deployer) internal {
        ...
        console.log("Note: This SecurityToken implementation includes EIP-2612 permit functionality for meta-transactions");
        ...
        registry.registerImplementation("security-token", "1.0.0", address(tokenImpl), "Security token with EIP-2612 meta-transactions");
    ...
    function _phaseThree_DeployToken(address deployer) internal {
        console.log("\n=== PHASE 3: Deploying Security Token with Meta-Transaction Support ===");
        ...
        console.log(" - EIP-2612 Permit Support: ENABLED");
    ...
    function _phaseFour_VerifyAndConfigureToken(address deployer) internal {
        ...
        // Verify EIP-2612 permit functionality
        console.log("Verifying EIP-2612 permit functionality...");
        try token.DOMAIN_SEPARATOR() returns (bytes32 domainSeparator) {
            console.log("Domain separator:", vm.toString(domainSeparator));
            console.log("[OK] EIP-2612 permit functionality is available");
        } catch {
            console.log("WARNING: Failed to verify DOMAIN_SEPARATOR");
            console.log("EIP-2612 permit functionality may not be available");
        }
        ...
        // Get the actual domain separator for display
        bytes32 domainSeparatorValue;
        try token.DOMAIN_SEPARATOR() returns (bytes32 separator) {
            domainSeparatorValue = separator;
        } catch {
            domainSeparatorValue = bytes32(0);
        }
        ...
        console.log("Users can sign permits off-chain for gasless approvals");
        console.log("Domain Separator:", vm.toString(domainSeparatorValue));
        console.log("\nTo use meta-transactions:");
        console.log("1. Users sign permits off-chain using EIP-712");
        console.log("2. Relayers submit permits on-chain");
        console.log("3. Transfers can be executed without user paying gas");
    }
}